...
async function searchBingForPristineImage(page, productName) {
...
async function batchDownloadPristineCatalog() {
  ...
  async function worker(workerId, page) {
    while (index < products.length) {
      ...
      console.log(`[Worker ${workerId}] [${currentIdx + 1}/${products.length}] ${p.name}`);
      const candidateUrls = await searchBingForPristineImage(page, p.name);

      let downloaded = false;
      for (const url of candidateUrls.slice(0, 5)) {
        const ok = await downloadImage(url, localDiskPath);
        if (ok && fs.existsSync(localDiskPath) && fs.statSync(localDiskPath).size > 4000) {
          downloaded = true;
          console.log(`✅ [Worker ${workerId}] Saved ${targetFilename} (${Math.round(fs.statSync(localDiskPath).size / 1024)} KB)`);
          break;
        }
      }
  ...
  await browser.close();
  console.log(`\n🎉 Processed ${successCount}/${products.length} products with pristine studio photography!`);
}

batchDownloadPristineCatalog().catch(console.error);
