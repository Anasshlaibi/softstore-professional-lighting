/**
 * convert_conviction_to_pdf.cjs
 *
 * Converts VALUE_DEFENSE_AND_CLIENT_CONVICTION_STRATEGY.md into a PDF document
 * using headless Microsoft Edge / Chrome.
 */
...
<head>
  <meta charset="UTF-8">
  <title>Master Client Conviction & Value Defense Strategy</title>
  <style>
...
<body>
  <div class="header-box">
    <div class="badge">High-Stakes Client Negotiation Playbook</div>
    <h1>Master Client Conviction & Value Defense Strategy</h1>
    <p>How to Defend Premium Pricing, Prove ROI & Overcome Lowball Objections</p>
  </div>
  ${bodyHtml}
</body>
</html>`;
...
async function run() {
  ...
  const mdPath = path.join(rootDir, 'VALUE_DEFENSE_AND_CLIENT_CONVICTION_STRATEGY.md');
  const htmlPath = path.join(rootDir, 'VALUE_DEFENSE_AND_CLIENT_CONVICTION_STRATEGY.html');
  const pdfPath = path.join(rootDir, 'VALUE_DEFENSE_AND_CLIENT_CONVICTION_STRATEGY.pdf');

  console.log('Reading VALUE_DEFENSE_AND_CLIENT_CONVICTION_STRATEGY.md...');
  ...
  console.log('Exporting Conviction PDF using headless browser...');
  const cmd = `"${browserPath}" --headless --disable-gpu --no-pdf-header-footer --print-to-pdf="${pdfPath}" "${htmlPath}"`;
  
  try {
    execSync(cmd, { stdio: 'inherit' });
    console.log(`\n🎉 Conviction PDF Successfully Generated!`);
    console.log(`   Location: ${pdfPath}`);
  } catch (err) {
    console.error('Failed to generate conviction PDF:', err.message);
    process.exit(1);
  }
}
